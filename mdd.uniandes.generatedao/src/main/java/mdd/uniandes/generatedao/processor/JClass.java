...
public class JClass {
    ...
    public JClass addFields(LinkedHashMap<String, String> identifierToTypeMap) {
        for (Map.Entry<String, String> entry : identifierToTypeMap.entrySet()) {
            addField(entry.getValue(), entry.getKey(),false);
        }
        return this;
    }

    public JClass addField(String type, String identifier,Boolean isId) {
        fields.put(identifier, type);
        if(isId) {
        	builder.append("@Id @GeneratedValue private ")
            .append(type)
            .append(" ")
            .append(identifier)
            .append(";")
    ...
    public JClass addConstructor(String accessModifier, List<String> fieldsToBind,List<String> bind) {
    ...
    public JClass createDaoMethods(String entityName) {
        addMethod(new JMethod()
                .defineSignature("public", false, "void")
                .name("save" + Character.toUpperCase(entityName.charAt(0)) + entityName.substring(1))
                .addParam(entityName.charAt(0) + entityName.substring(1),entityName.toLowerCase())
                .defineBody("EntityTransaction trans = em.getTransaction();\n" + 
                		"		trans.begin();\n" + 
                		"		em.persist("+entityName.toLowerCase()+");\n" + 
                		"		trans.commit();"));
        
        addMethod(new JMethod()
                .defineSignature("public", false, "List<"+Character.toUpperCase(entityName.charAt(0)) + entityName.substring(1)+">")
                .name("get" + Character.toUpperCase(entityName.charAt(0)) + entityName.substring(1))
                .defineBody("Query q = em.createQuery(\"SELECT "+Character.toLowerCase(entityName.charAt(0))+" FROM "+Character.toUpperCase(entityName.charAt(0)) + entityName.substring(1)+" "+Character.toLowerCase(entityName.charAt(0))+"\", "+Character.toUpperCase(entityName.charAt(0)) + entityName.substring(1)+".class);\n" + 
                		"List<"+Character.toUpperCase(entityName.charAt(0)) + entityName.substring(1)+"> results = q.getResultList();\n" + 
                		"return results;"));
